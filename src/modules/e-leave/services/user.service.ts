...
export class UserService {
  ...
  async updatePassword(id: EmployeeID, updatePassword: UpdatePasswordDto) {

    const user = await this.UserRepository.$findOne('employee_id', id?.employee_id);

    if (!user) {
      throw new Error('User not found');
    }

    const password = await bcrypt.hash(updatePassword?.password, 10);
